    ...
    for (spp in 1:nspp){
      if (spp < nspp){
        for (spp_next in (spp + 1):nspp){
          incidence[spp,spp_next] <- sum(site_mask[spp,]*site_mask[spp_next,]*spp_site_mat[spp,])
          incidence[spp_next,spp] <- sum(site_mask[spp,]*site_mask[spp_next,]*spp_site_mat[spp_next,])
        }
      }
    }
      ...
      if (spp < nspp){
        for (spp_next in (spp + 1):nspp){
          ...
          if(is.na(prob_occur[spp, spp_next])){
            ...
            pairs <- sum(as.numeric(spp_site_mat[spp,site_mask[spp,]*site_mask[spp_next,]==1]==1&spp_site_mat[spp_next,site_mask[spp,]*site_mask[spp_next,]==1]==1))
            #pairs <- 0
